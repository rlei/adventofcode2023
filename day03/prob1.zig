...
pub fn main() !void {
    ...
    var symbolPosList = std.ArrayList(Pos).init(allocator);
    defer symbolPosList.deinit();
    ...
    while (true) {
        ...
        if ((ch >= '0') and (ch <= '9')) {
            ...
            if (ch != '.') {
                try symbolPosList.append(Pos{.row=row, .col=col});
                // std.debug.print("symbol {c} at ({d}, {d})\n", .{ch, row, col});
            }
        }
        col += 1;
    }
    ...
    while (iter.next()) |partNumberId| {
        ...
        // std.debug.print("number id {d} value {d}\n", .{partNumberId.*, part});
        sum += part;
    }
