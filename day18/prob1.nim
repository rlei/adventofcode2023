...
type
  ...
  FoldState = tuple[inside: bool, acc: int, lastShape: char, lastCol: int]
...
proc updatePosToTurn(pos: Pos, dir: Dir) =
    if posToMove.hasKey(pos):
        let (lastMove, thisMove) = 
            if pos != (row: 0, col: 0): (posToMove[pos], dir)
...
for line in stdin.lines:
    var move: char
    var distance, color: int
    if line.scanf("$c $i (#$h)", move, distance, color):
        borderLen += distance
        var dir = case move
            of 'U': Dir.Up
            of 'D': Dir.Down
            of 'L': Dir.Left
            of 'R': Dir.Right
            else: raise newException(ValueError, "Invalid move direction!")
