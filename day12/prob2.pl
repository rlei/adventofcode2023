...
count_matches_5x(Pattern, Counts5x, Matches) :-
  ...
  string_chars(Pattern5x, Chars),
  gen_pattern(Chars, NewChars, 0),
  string_chars(NewPattern5x, NewChars),
  split_string(NewPattern5x, ".", ".", Pattern5xList),
  % writeln(["Running => ", NewPattern5x, "  ", Counts5x]),
  aggregate(sum(EachMatch), (count_matches_dp(Pattern5xList, Counts5x, EachMatch)), Matches).
  % writeln([NewPattern5x, "  ", Counts5x, "  ", Matches]).

gen_pattern([], [], _).
gen_pattern(['#'|RestPattern], [OutH|OutTail], _) :-
  OutH = '#',
  gen_pattern(RestPattern, OutTail, 0).
gen_pattern(['.'|RestPattern], [OutH|OutTail], _) :-
  OutH = '.',
  gen_pattern(RestPattern, OutTail, 0).
gen_pattern(['?'|RestPattern], [OutH|OutTail], NumUnknown) :-
  (NumUnknown = 5 ->
    (OutH = '#'; OutH = '.'),
    NewNumUnknown is 0;
    % --- else ---
    OutH = '?',
    NewNumUnknown is NumUnknown + 1),
  gen_pattern(RestPattern, OutTail, NewNumUnknown).
